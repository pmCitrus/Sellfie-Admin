...
@push('scripts')
        ...
        $(document).ready(function()
	{
            ...
            $(document.body).on('click', '#view_row', function (e){
                var url     = "{{ route('kyc', ['kyc_status' => Request::segment(2)]) }}/"+$(this).attr('name')+"/view";
                window.location.replace(url);
            });
            
            $(document.body).on('click', '#approve_row, #reject_row', function (e){
                var name        = $(this).attr('name');
                var action_type = ($(this).attr('id') == 'approve_row') ? 'approve' : 'reject';
                
                if(confirm('Do you want to '+action_type+' kyc?'))
                {
                    var url     = "{{ route('kyc', ['kyc_status' => Request::segment(2)]) }}/"+name+"/"+action_type;
                    window.location.replace(url);
                }
                else
                {
                    ...
                    },
                    {
                        ...
                        "render": function ( data, type, full ) {
                                    return  ' <a name="'+full.kyc_ref_id+'" id="view_row"'
                                            ...
                                            + '</a>&nbsp;'
                                            +'<a name="'+full.kyc_ref_id+'" id="approve_row"'
                                            + ' class="btn btn-xs btn-success">'
                                            + 'Approve'
                                            + '</a>&nbsp;'
                                            +'<a name="'+full.kyc_ref_id+'" id="reject_row"'
